document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('download').addEventListener('click', () => {
        ...
        if (currentTab === 'recolor') {
            ...
            menu.innerHTML = `
                <button onclick="downloadFile('jpeg')">JPEG</button>
                <button onclick="downloadFile('png')">PNG</button>
                <button onclick="downloadFile('svg')">SVG</button>
    ...
    window.downloadFile = function(format) {
        ...
        if (format === 'svg') {
            ...
            const doc = new jsPDF();
            doc.addSvgAsImage(svg.outerHTML, 0, 0, svg.width.baseVal.value, svg.height.baseVal.value);
